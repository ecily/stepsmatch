...
export default function AddOfferForm() {
  ...
  useEffect(() => {
    ...
    axiosInstance.get(`providers/${resolvedProviderId}`)
      .then((res) => {
  ...
  // >>> FIX: Upload via fetch (kein Axios, kein Content-Type Header manuell!)
  const handleImageChange = async (e) => {
    ...
    try {
      ...
      files.forEach((file) => fd.append('images', file)); // Feld MUSS "images" heißen

      const resp = await fetch(`${API_BASE}/uploads/images?folder=offers`, {
        method: 'POST',
        body: fd,
        // KEINE headers setzen! Browser setzt multipart boundary selbst.
      });
      ...
      // WICHTIG: kein e.target.value Reset → Dateiname bleibt sichtbar
    } catch (err) {
  ...
  const removeImage = async (index) => {
    ...
    try {
      ...
      // optional optimistisches Entfernen:
      setFormData((prev) => ({
        ...prev,
        images: prev.images.filter((_, i) => i !== index),
      }));
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white shadow-lg rounded-xl mt-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Gültig ab / bis</label>
