...
/**
 * Filter & Dedupe + deaktivierte Tokens aussortieren (aus der DB)
 */
export async function filterValidActiveTokens(tokens = []) {
  ...
  const dbRows = await PushToken.find(
    { token: { $in: maybeValid } },
    { token: 1, disabled: 1 },
  ).lean();
...
export async function sendOffersPushAsync(
  tokens,
  {
    ...
    channelId = 'offers',
    sound = 'default',
    categoryId = 'offers-actions', // Buttons sichtbar machen
  }
) {
 ...
 * Mit categoryId + auto data.enrichment.
 */
