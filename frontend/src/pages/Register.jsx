...
const Register = ({ onRegisterSuccess }) => {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      if (!userId) throw new Error("userId fehlt in der Registrierungs-Antwort");

      localStorage.setItem("userId", userId);

      if (onRegisterSuccess) onRegisterSuccess(userId);
      // Weiter zum Anbieter-Wizard
      navigate("/add-provider");
    } catch (err) {
      console.error(err);
      setError(
        err?.response?.data?.error || err?.message || "Registrierung fehlgeschlagen."
      );
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="min-h-[92vh] md:min-h-screen bg-gradient-to-b from-blue-50 via-white to-white flex items-start md:items-center justify-center px-6 py-10">
      <div className="w-full max-w-2xl">
        ...
        <header className="relative overflow-hidden rounded-2xl border border-blue-100 bg-gradient-to-br from-blue-50 to-indigo-50 p-6 md:p-8 shadow-sm">
          <div
            className="absolute -top-16 -right-16 h-56 w-56 rounded-full bg-blue-200/30 blur-3xl"
            aria-hidden
          />
          <div className="relative">
            <h1 className="text-2xl md:text-3xl font-extrabold tracking-tight text-blue-900">
              Anbieter registrieren · Vorschau
            </h1>
            <p className="mt-2 text-gray-700">
              ...
              <b>Echtzeit-KPIs</b> (z. B. „Angebot zugestellt“, „Route gestartet“, „Angekommen“) und
              bekommst <b>volle Transparenz</b> darüber, wie StepsMatch dir hilft, deinen{" "}
              <b>Umsatz zu steigern</b>.
            </p>
        ...
        <div className="mt-6 rounded-2xl bg-white border border-gray-100 shadow p-6 md:p-8">
          <h2 className="text-xl md:text-2xl font-bold text-gray-900">Konto erstellen</h2>
          <p className="mt-1 text-sm text-gray-600">
            Lege dein Anbieter-Profil an und starte in wenigen Minuten.
          </p>
        ...
        <p className="mt-4 text-xs text-gray-500">
          In der finalen Version folgen geführtes Anbieter-Onboarding, Angebots-Assistent,
          Echtzeit-KPIs mit Drilldown (Zugestellt → Route gestartet → Angekommen) und transparente
